...
export default function ListenPage() {
    ...
    useEffect(() => {
        if (playerState !== 'playing' && playerState !== 'paused' && playerState !== 'buffering') return;
    ...
    return (
        <div className="min-h-[calc(100vh-8rem)] flex flex-col items-center justify-center px-6 py-8 relative overflow-hidden">
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[150%] h-[150%] pointer-events-none opacity-20">
                <div className={`w-full h-full rounded-full blur-[120px] transition-colors duration-1000 ${playerState === 'playing' ? 'bg-primary-600'
                    : playerState === 'waiting' ? 'bg-amber-600'
                        : 'bg-surface-800'
                    }`} />
            </div>
                ...
                <div className="flex flex-col items-center gap-2">
                    ...
                    {playerState === 'buffering' && (
                        <div className="px-3 py-1 bg-amber-500/10 border border-amber-500/20 rounded-full flex items-center gap-2">
                            <svg className="w-3 h-3 text-amber-500 animate-spin" viewBox="0 0 24 24">
                            ...
                            <span className="text-amber-500 text-[10px] font-bold uppercase tracking-widest leading-none">
                                {retryIn > 0 ? `Riconnessione in ${retryIn}s...` : 'Connessione...'}
                            </span>
                        </div>
                    )}
                    {(playerState === 'offline') && (
                        <div className="px-3 py-1 bg-surface-800 border border-surface-700 rounded-full flex items-center gap-2">
                            ...
                            <span className="text-surface-400 text-[10px] font-bold uppercase tracking-widest leading-none">Diretta terminata</span>
                        </div>
                    )}
                    {playerState === 'waiting' && (
                        <div className="px-3 py-1 bg-amber-500/5 border border-amber-500/20 rounded-full flex items-center gap-2">
                            <span className="w-2 h-2 bg-amber-500 rounded-full animate-pulse" />
                            <span className="text-amber-500/80 text-[10px] font-bold uppercase tracking-widest leading-none">In attesa della diretta...</span>
                        </div>
                    )}
                </div>
            </div>
                        ...
                        {playerState === 'loading' ? (
                            ...
                            <div className="flex flex-col items-center gap-3">
                                ...
                                <div className="flex gap-1">
                                    <span className="w-1.5 h-1.5 bg-amber-500/40 rounded-full animate-bounce" style={{ animationDelay: '0ms' }} />
                                    <span className="w-1.5 h-1.5 bg-amber-500/40 rounded-full animate-bounce" style={{ animationDelay: '150ms' }} />
                                    <span className="w-1.5 h-1.5 bg-amber-500/40 rounded-full animate-bounce" style={{ animationDelay: '300ms' }} />
                                </div>
                            </div>
                        ) : playerState === 'offline' ? (
                            ...
                            <div className="flex flex-col items-center gap-2">
                                <svg className="w-8 h-8 text-amber-500 animate-spin" viewBox="0 0 24 24">
                                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" />
                                    <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
                                </svg>
                                {retryCountRef.current > 0 && (
            ...
            <div className="w-full max-w-xs space-y-4 z-20 mt-4">
                {playerState === 'offline' ? (
                    ...
                    <div className="space-y-4">
                        ...
                        <button onClick={handleRetry} className="btn-primary w-full py-4">Riprova</button>
                    </div>
                ) : playerState === 'waiting' ? (
                    <div className="flex flex-col items-center gap-4">
                        <p className="text-surface-500 text-xs text-center font-medium">
                    ...
                    <div className="flex flex-col items-center gap-8">
                        ...
                        <p className="text-surface-500 text-[10px] uppercase font-bold tracking-widest flex items-center gap-2">
                            ...
                            {playerState === 'playing' ? 'Audio Streaming Attivo'
                                : playerState === 'buffering' ? 'Connessione in corso...'
